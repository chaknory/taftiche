    ...
    if ($method === 'GET') {
        $stmt = $pdo->query(
            "SELECT id, first_name, family_name, maiden_name, gender, birth_date,
                    birth_place, residence, marital_status, spouse_name,
                    phone, email, address,
    ...
    if ($method === 'PUT') {
        ...
        $pdo->prepare(
            "UPDATE personal_info SET
                ...
                marital_status=:ms, spouse_name=:sn, phone=:ph, email=:em,
                address=:addr, district=:di, school_year=:sy, school_name=:sname,
                years_worked=:yw, school_entry_date=:sed, diploma=:dip
             WHERE id = :id"
        )->execute([
            ':fn'=>$firstName, ':fam'=>$familyName, ':mn'=>$maidenName,
            ':gen'=>$gender, ':bd'=>$birthDate, ':bp'=>$birthPlace, ':res'=>$residence,
            ':ms'=>$maritalStatus, ':sn'=>$spouseName, ':ph'=>$phone, ':em'=>$email,
            ':addr'=>$address, ':di'=>$district, ':sy'=>$schoolYear, ':sname'=>$schoolName,
            ':yw'=>$yearsWorked, ':sed'=>$schoolEntry, ':dip'=>$diploma, ':id'=>$id,
        ]);
